...
/* Palette is mapped to the /home dark theme (brown-on-cream with copper
   accent) so the manifesto and the product page feel like the same product.
   The names are kept as --green/--paper/--ink for minimal diff with the
   original component, but the values are now the brown palette. */
.langship-root {
  --ink: #1a120c;          /* deep brown ink for the letter card */
  --paper: #E9DECF;        /* warm cream — matches /home --fg */
  --paper-shadow: #D6C7B0; /* slightly deeper cream for the letter drop shadow */
  --dark-bg: #13100E;      /* /home dark --bg */
  --dark-surface: #1B1715; /* /home dark --bg-2 */
  --green: #D08758;        /* copper accent — matches /home --accent (was #C96A5A) */
  --green-dim: #8C5230;    /* darker copper for hover/dim states */
  --red: #d97757;          /* warm error tone, not pure red, to stay on-palette */
  --muted: #9B8E7E;        /* warm muted — matches /home --muted-fg */
...
.hero h1 { font-family: var(--letter-font); font-size: clamp(3.5rem, 10vw, 7rem); font-weight: 700; color: var(--paper); line-height: 1.05; margin-bottom: 1rem; opacity: 0; animation: fadeUp 0.7s ease forwards 0.5s; }
.hero-sub { font-family: var(--letter-font); font-size: clamp(1.2rem, 3vw, 1.6rem); color: var(--muted); font-weight: 400; max-width: 600px; line-height: 1.7; opacity: 0; animation: fadeUp 0.7s ease forwards 0.7s; }
.hero-sub em { color: var(--paper); font-style: italic; font-weight: 600; }
.hero-aside { display: inline-block; margin-top: 0.25rem; font-size: 0.9em; color: var(--green); font-style: italic; }
.scroll-hint { position: absolute; bottom: 2.5rem; left: 50%; transform: translateX(-50%); font-size: 0.7rem; letter-spacing: 0.2em; color: #5a4d40; text-transform: uppercase; opacity: 0; animation: fadeIn 1s ease forwards 1.5s; }
.scroll-hint::after { content: ''; display: block; width: 1px; height: 40px; background: linear-gradient(to bottom, #5a4d40, transparent); margin: 0.75rem auto 0; animation: pulse 2s ease-in-out infinite; }

.letter-wrap { position: relative; z-index: 1; max-width: 780px; margin: 0 auto; padding: 0 1.5rem; }
.letter { background: var(--paper); color: var(--ink); padding: 4rem 4rem 3.5rem; border-radius: 3px; position: relative; margin: 4rem 0; box-shadow: 0 1px 0 var(--paper-shadow), 0 4px 0 #d5ccb8, 0 8px 24px rgba(0,0,0,0.6); }
...
.questions-header { font-family: var(--letter-font); font-size: 1.1rem; color: var(--muted); margin-bottom: 2rem; display: flex; align-items: center; gap: 0.75rem; }
.questions-header::before, .questions-header::after { content: ''; flex: 1; height: 1px; background: rgba(255,255,255,0.08); }
...
.q-text { font-family: var(--letter-font); font-size: 1.5rem; line-height: 1.4; color: var(--paper); }
...
.terminal { background: var(--dark-surface); border: 1px solid rgba(255,255,255,0.07); border-radius: 8px; overflow: hidden; margin: 4rem auto; max-width: 780px; position: relative; z-index: 1; }
.terminal-bar { background: #1a1a1a; border-bottom: 1px solid rgba(255,255,255,0.06); padding: 0.7rem 1rem; display: flex; align-items: center; gap: 0.5rem; }
.dot { width: 11px; height: 11px; border-radius: 50%; }
.dot-r { background: #ff5f57; } .dot-y { background: #febc2e; } .dot-g { background: #28c840; }
.terminal-title { font-size: 0.72rem; color: #6e6055; letter-spacing: 0.05em; margin-left: 0.5rem; }
.terminal-body { padding: 1.5rem 1.75rem; font-family: var(--mono); font-size: 0.82rem; line-height: 1.9; }
.t-comment { color: #5a4d40; } .t-prompt { color: #6e6055; margin-right: 0.5rem; } .t-cmd { color: #d3c7b6; }
.t-ok { color: #8FB069; } .t-err { color: var(--red); } .t-muted { color: #6e6055; }
...
.section-title { font-family: var(--letter-font); font-size: clamp(2rem, 5vw, 3rem); color: var(--paper); margin-bottom: 3rem; line-height: 1.2; }
.section-title em { font-style: italic; color: var(--muted); }
.features-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 1px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.06); border-radius: 8px; overflow: hidden; }
.feat-card { background: var(--dark-bg); padding: 1.75rem 1.5rem; transition: background 0.2s; }
.feat-card:hover { background: var(--dark-surface); }
...
.feat-desc { font-size: 0.8rem; color: #8a7a6a; line-height: 1.6; }
...
.comp-table th:first-child { color: #6e6055; }
.comp-table th:nth-child(2) { color: #6e6055; text-align: center; }
.comp-table th:nth-child(3) { color: var(--green); text-align: center; }
.comp-table td { padding: 0.9rem 1rem; border-top: 1px solid rgba(233,222,207,0.06); color: #b3a695; }
...
.yes { color: #8FB069; font-size: 1rem; } .no { color: var(--red); font-size: 0.75rem; letter-spacing: 0.05em; } .paid { color: #9b8e7e; font-size: 0.7rem; letter-spacing: 0.05em; }
...
.cta-pre { font-family: var(--letter-font); font-size: 1.1rem; color: var(--muted); margin-bottom: 1rem; }
.cta-title { font-family: var(--letter-font); font-size: clamp(2.5rem, 7vw, 4.5rem); font-weight: 700; color: var(--paper); line-height: 1.1; margin-bottom: 1rem; }
.cta-sub { font-family: var(--mono); font-size: 0.8rem; color: #6e6055; margin-bottom: 2.5rem; letter-spacing: 0.05em; }
...
.cta-fine { margin-top: 1.25rem; font-size: 0.72rem; color: #5a4d40; letter-spacing: 0.05em; }
...
.enter-btn:hover { background: rgba(255,255,255,0.04); border-color: rgba(255,255,255,0.32); transform: translateY(-2px); }

.langship-root footer { border-top: 1px solid rgba(255,255,255,0.05); padding: 2.5rem; text-align: center; position: relative; z-index: 1; }
.footer-sign { font-family: var(--letter-font); font-size: 2rem; color: var(--paper); margin-bottom: 0.5rem; }
.footer-url { font-family: var(--mono); font-size: 0.75rem; color: #5a4d40; letter-spacing: 0.1em; }
        ...
        <div className="terminal reveal">
          ...
          <div className="terminal-body">
            ...
            <div className="t-line" style={{ marginTop: "1.25rem" }}>
              <span className="t-prompt">$</span>
              <span className="t-cmd">
                pip install langship<span className="blink"></span>
              </span>
            </div>
          </div>
        </div>
